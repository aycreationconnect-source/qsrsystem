...
// 1. Replace the state
const oldStateRegex = /const \[newItem, setNewItem\] = useState\(\{ name: '', price: '', type: 'Veg' \}\);/;
const newStateCode = `const [newItem, setNewItem] = useState<any>({ name: '', category: '', description: '', image: '', price: '', tax: '', sku: '', prepTime: '', type: 'Veg', available: true, status: 'Active' });`;
appCode = appCode.replace(oldStateRegex, newStateCode);

// 2. Replace handleEditItem
const oldEditRegex = /const handleEditItem = \([\s\S]*?setShowAddItemModal\(true\);\s*\};/;
const newEditCode = `const handleEditItem = (item: any) => {
    setNewItem({
      name: item.name || '',
...
appCode = appCode.replace(oldEditRegex, newEditCode);

// 3. Replace Add Item Modal
const oldModalRegex = /\{\/\* Add Item Modal \*\/\}\s*\{showAddItemModal && \([\s\S]*?<\/datalist>\s*<div className="form-group">\s*<label>Item Name<\/label>[\s\S]*?\}\)\}>Save Item<\/button>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*\)\}/;

const newModalCode = `{/* Add Item Modal */}
          {showAddItemModal && (
            <div className="modal-overlay">
              <div className="modal-content" style={{ width: 650, maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
                <div className="modal-header">
                  ...
                  <button className="close-btn" onClick={() => { setShowAddItemModal(false); setEditingItemIndex(null); setNewItem({ name: '', category: '', description: '', image: '', price: '', tax: '', sku: '', prepTime: '', type: 'Veg', available: true, status: 'Active' }); }}>&times;</button>
                </div>
                <div className="modal-body" style={{ overflowY: 'auto', paddingRight: 8 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    ...
                    <div className="form-group">
                      ...
                      <select style={{ width: '100%', padding: '12px', background: '#fff', border: '1px solid var(--border-color)', borderRadius: 8 }} value={newItem.category || selectedCategory || ''} onChange={(e) => setNewItem({...newItem, category: e.target.value})} required>
                        ...
                        {appData.categories.map((c:any, i:number) => {
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
                    ...
                    <div className="form-group">
                      ...
                      <input type="file" accept="image/*" onChange={(e) => {
                        // Temporary dummy image logic since no backend upload yet
                  ...
                  <div className="form-group" style={{ marginTop: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }} onClick={() => setShowIngredientsList(!showIngredientsList)}>
                      <input type="checkbox" checked={showIngredientsList} readOnly />
                      <span style={{ fontWeight: 600 }}>Manage Recipe / Ingredients (Optional)</span>
                    </div>
                    {showIngredientsList && (
                      <div className="ingredients-list" style={{ marginTop: 12, padding: 16, background: '#f8fafc', borderRadius: 8, border: '1px solid var(--border-color)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                           <h4 style={{ margin: 0, fontSize: '0.9rem' }}>Ingredients List</h4>
                  ...
                  <div style={{ display: 'flex', gap: 16, marginTop: 24 }}>
                    <button className="btn btn-prev" style={{ flex: 1 }} onClick={() => { setShowAddItemModal(false); setEditingItemIndex(null); setNewItem({ name: '', category: '', description: '', image: '', price: '', tax: '', sku: '', prepTime: '', type: 'Veg', available: true, status: 'Active' }); }}>Cancel</button>
                    <button className="btn btn-next" style={{ flex: 1 }} onClick={() => {
                      ...
                      if (editingItemIndex !== null) {
                        newAppData.menu[editingItemIndex] = finalItem;
                      } else {
                      ...
                      setNewItem({ name: '', category: '', description: '', image: '', price: '', tax: '', sku: '', prepTime: '', type: 'Veg', available: true, status: 'Active' });
...
console.log('App.tsx refactored for new item fields');
